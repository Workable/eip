...
Timeout.prototype.connect = function () {
  amqplib.connect(this.queue, {
    ...
    .then(function (conn) {
      ...
      this.connection.on('close', logger.error.bind(logger, 'Connection to rabbitmq closed...'));
      this.connection.on('error', logger.error.bind(logger, 'Connection error with rabbitmq: '));
...
Timeout.prototype.inject = function inject(correlationId, cb) {
  ...
  this.channel.sendToQueue(this.queueName, new Buffer(correlationId), {persistent: true}, cb);
};
