...
var setErrorOnEvent = function (err, event, processor) {
  ...
  config.logger.error("Error occured for event %s\n Number of attempts: %d; error: %s.",
...
Route.prototype.dispatchEvent = function (err, event, processor) {
  ...
  if (err) {
    ...
    if (event.headers._exception.numberOfAttempts < retryLimit) {
      var self = this
        , delay = config.route.retryDelay;
      config.logger.error("Retry limit not reached, try again in %d ms.", delay);
      setTimeout(function () {
...
Route.prototype._sendEventToErrorRoute = function (event) {
  if (this.errorRoute) {
    config.logger.warn("Retry limit reached, send event to error route. Event: %j", event);
    this.errorRoute.inject(event);
  } else {
    config.logger.error("Unrecoverable error in error route, give up processing event %j.", event);
  }
};
...
//	config.logger.log("Trying to shut down processor %s.", procMeta.processor.id);
...
//		config.logger.log("Shutting down processor %s must be delayed because of non-empty in-queue.",
//				procMeta.processor.id);
    // Since there are still events to be sent to the processor, wait a moment and try again:
    process.nextTick(function () {
      tryShutDown(procMeta, callback);
    });
  }
};
...
//	config.logger.log("Shutting down processor %s.", this.id);
  callback();
};
...
process.on('uncaughtException', function (err) {
  config.logger.error("An uncaught exception occured! If possible, the error will be sent to the global error route.");
  ...
  if (globalErrorRoute) {
    ...
    config.logger.log("About to exit");
    process.exit(8);
  }
});*/
