...
Aggregator.prototype.initTimeout = function () {
  if (this.options.queue) {
    Timeout.connect(this.options.queue);
    return new Timeout(this.route, this.options.timeoutDelay || 60000, function (msg, ack) {
    ...
    return {
      inject: function (msg, cb) {
        ...
        }.bind(this), this.options.timeoutDelay || 60000);
        cb();
      }.bind(this)
    }
  }
...
Aggregator.prototype.genEmitEventByCorrelationId = function * (correlationId, completed) {
  try {
    ...
    if (this.persist) {
      ...
      if (dbRecord.status === 'completed' || (dbRecord.status === 'expired' && completed)) {
