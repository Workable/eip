...
AggregationStrategy.prototype.emit = function (event, complete) {
  complete = complete !== false;
  this.aggregator.emitEventByCorrelationId(this.aggregator.options.correlator(event), complete)
};
  ...
  try {
    ...
    if (this.persist) {
      ...
      if (dbRecord.status === 'completed') {
        config.logger.info('already completed: ' + correlationId);
        return;
      }
...
Aggregator.prototype.shutDown = function () {
  config.logger.info("Shutting down aggregator...");
};
...
Aggregator.initDB = function (options) {
  ...
  db.on('error', function (err) {
    config.logger.error('unable to connect to database', err);
    throw err;
  });
}
